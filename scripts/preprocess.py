...
def preprocess(model, filepath):
    ...
    if model.config["class"] == "polarity":
        ...
        raw_df = raw_df.loc[(raw_df["intensity"] > 0) |
                            (raw_df["polarity"] > 0)]
...
def tokenize(model, train_df, val_df, test_df):
    train_tok = dict(
        model.tokenizer(train_df["tweet"].values.tolist(),
                        truncation=True,
                        padding="max_length",
    ...
    val_tok = dict(
        model.tokenizer(val_df["tweet"].values.tolist(),
                        truncation=True,
                        padding="max_length",
                        max_length=model.config["max_length"],
                        return_tensors="tf",
                        return_token_type_ids=False))
    test_tok = dict(
        model.tokenizer(test_df["tweet"].values.tolist(),
                        truncation=True,
                        padding="max_length",
                        max_length=model.config["max_length"],
                        return_tensors="tf",
    ...
    train_ds = tensorflow.data.Dataset.from_tensor_slices(
        (train_tok,
         train_df[model.config["class"]])).batch(model.config["batch_size"])
    val_ds = tensorflow.data.Dataset.from_tensor_slices(
        (val_tok,
         val_df[model.config["class"]])).batch(model.config["batch_size"])
    test_ds = tensorflow.data.Dataset.from_tensor_slices(
        (test_tok,
         test_df[model.config["class"]])).batch(model.config["batch_size"])
