...
hypnox.watchdog.script_log.info("loading input labeled data")
...
hypnox.watchdog.script_log.info("retrieving highest scored tweets set")
...
hypnox.watchdog.script_log.info("fetching 200 most frequent words in the set")
...
hypnox.watchdog.script_log.info("splitting data into 1000 lines chunks")
output_chunks = [
    dup_filtered.values[i:i + 1000]
    for i in range(0, len(dup_filter.values), 1000)
]

hypnox.watchdog.script_log.info("saving to file")
