...
def train(args):
    ...
    train_df, test_df, = sklearn.model_selection.train_test_split(
        raw_df, stratify=raw_df[model_config.yaml["class"]], test_size=model_config.yaml["test_size"], random_state=93)
    train_df, val_df = sklearn.model_selection.train_test_split(
        train_df, stratify=train_df[model_config.yaml["class"]], test_size=model_config.yaml["test_size"], random_state=93)

    # tokenize training data
    train_tok = dict(tokenizer(train_df["tweet"].values.tolist(), truncation=True, padding="max_length",
                     max_length=model_config.yaml["max_length"], return_tensors="tf", return_token_type_ids=False))
    val_tok = dict(tokenizer(val_df["tweet"].values.tolist(), truncation=True, padding="max_length",
                   max_length=model_config.yaml["max_length"], return_tensors="tf", return_token_type_ids=False))
    test_tok = dict(tokenizer(test_df["tweet"].values.tolist(), truncation=True, padding="max_length",
                    max_length=model_config.yaml["max_length"], return_tensors="tf", return_token_type_ids=False))

    # build tensorflow datasets
    train_ds = tensorflow.data.Dataset.from_tensor_slices(
        (train_tok, train_df[model_config.yaml["class"]])).batch(model_config.yaml["batch_size"])
    val_ds = tensorflow.data.Dataset.from_tensor_slices(
        (val_tok, val_df[model_config.yaml["class"]])).batch(model_config.yaml["batch_size"])
    test_ds = tensorflow.data.Dataset.from_tensor_slices(
        (test_tok, test_df[model_config.yaml["class"]])).batch(model_config.yaml["batch_size"])

    # build model
    input_ids = tensorflow.keras.layers.Input(
        shape=(model_config.yaml["max_length"],), name='input_ids', dtype='int32')
    mask = tensorflow.keras.layers.Input(
        shape=(model_config.yaml["max_length"],), name='attention_mask', dtype='int32')
    ...
    y = tensorflow.keras.layers.Dense(
        model_config.yaml["num_labels"], activation='softmax', name='outputs')(X)
    ...
    earlystop = tensorflow.keras.callbacks.EarlyStopping(
        monitor=model_config.yaml["val_loss"], patience=model_config.yaml["patience"], min_delta=model_config.yaml["min_delta"], verbose=1, mode="min", restore_best_weights=True)
    tensorboard = tensorflow.keras.callbacks.TensorBoard(
        log_dir=model_config.model_path + "/logs", histogram_freq=1)
    model.fit(train_ds, validation_data=val_ds, epochs=model_config.yaml["epochs"], callbacks=[
              earlystop, tensorboard], batch_size=model_config.yaml["batch_size"])
