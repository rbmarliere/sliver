...
# def get_balance_by_exchange_asset(user: User, asset: ExchangeAsset):
#     cred = user.get_active_credential(asset.exchange).get()
#     # TODO
#     # exchange = exchanges.load(cred.exchange, credential=cred)
#     # sync_balance(exchange)
#     return Balance.get(user_id=user.id, asset_id=asset.id)


# def get_balance_by_asset(user: User, asset: Asset):
#     balance = {
#         "ticker": asset.ticker,
#         "free": 0,
#         "used": 0,
...
# def get_target_cost(user_strat: UserStrategy):
...
#         ex_asset, new = ExchangeAsset.get_or_create(
...
# def sync_balances(user: User):
#     active_exchanges = []
#     # sync balance for each exchange the user has a key
#     for cred in user.credential_set.where(Credential.active):
