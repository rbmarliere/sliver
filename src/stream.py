...
class Stream(tweepy.Stream):
    def on_status(self, status):
        ...
        # format data
        tweet = {}
        tweet["time"] = datetime.datetime.strptime(
        ...
        try:
            ...
            output = pandas.DataFrame(
                {"time": [tweet["time"]], "tweet": text, "model_i": "", "intensity": 0, "polarity": 0, "model_p": ""})
...
def stream(args):
    ...
    if config.config["CONSUMER_KEY"] == "" or config.config["CONSUMER_SECRET"] == "" or config.config["ACCESS_KEY"] == "" or config.config["ACCESS_SECRET"] == "":
        logging.error(
            "empty keys in config! (CONSUMER_KEY, CONSUMER_SECRET, ACCESS_KEY, ACCESS_SECRET)")
        return 1
    auth = tweepy.OAuthHandler(
        config.config["CONSUMER_KEY"], config.config["CONSUMER_SECRET"])
    auth.set_access_token(
        config.config["ACCESS_KEY"], config.config["ACCESS_SECRET"])
    api = tweepy.API(auth)

    logging.info("initializing")
    stream = Stream(config.config["CONSUMER_KEY"], config.config["CONSUMER_SECRET"],
                    config.config["ACCESS_KEY"], config.config["ACCESS_SECRET"])
